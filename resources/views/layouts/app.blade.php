...
<body class="text-text dark:text-dark-text transition-colors duration-200" data-theme="{{ session('theme', 'light') }}">
            ...
            <div class="flex justify-between h-16">
                <div class="flex items-center">
                    <a href="{{ route('home') }}" class="flex items-center space-x-2">
                        ...
                        <span class="text-xl font-bold bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent hidden sm:inline">Perpustakaan Digital</span>
                    </a>
                </div>
                <div class="flex items-center space-x-2 sm:space-x-4">
                    ...
                    <a href="{{ route('home') }}" class="hidden sm:block text-text-muted hover:text-white transition-colors duration-300">Home</a>
                    <a href="{{ route('search') }}" class="hidden sm:block text-text-muted hover:text-white transition-colors duration-300">Search</a>
                    <a href="{{ route('history') }}" class="hidden sm:block text-text-muted hover:text-white transition-colors duration-300">History</a>
    ...
    <div id="mobileDrawer" class="drawer fixed inset-y-0 right-0 w-72 glass z-50 flex flex-col">
        ...
        <div class="flex-1 p-4 space-y-2">
            <a href="{{ route('home') }}" onclick="toggleDrawer()" class="flex items-center space-x-3 p-3 rounded-lg glass-input hover:bg-surface-hover transition-all duration-300">
                <svg class="w-5 h-5 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path>
                </svg>
                <span>Home</span>
            </a>
            <a href="{{ route('search') }}" onclick="toggleDrawer()" class="flex items-center space-x-3 p-3 rounded-lg glass-input hover:bg-surface-hover transition-all duration-300">
                <svg class="w-5 h-5 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                </svg>
                <span>Search</span>
            </a>
            <a href="{{ route('history') }}" onclick="toggleDrawer()" class="flex items-center space-x-3 p-3 rounded-lg glass-input hover:bg-surface-hover transition-all duration-300">
